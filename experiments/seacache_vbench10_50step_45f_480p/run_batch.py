...
def main():
    parser = argparse.ArgumentParser(description="Single-process Vbench10 SeaCache Wan2.2 T2V runner")
    ...
    if not torch.cuda.is_available():
        raise SystemExit("CUDA is not available; Vbench10 SeaCache runner requires GPU mode.")

    root_dir = Path(args.root_dir)
    if args.exp_root is None:
        stamp = time.strftime("%Y%m%d_%H%M%S")
        args.exp_root = f"/hy-tmp/wan22_seacache_vbench10_50step_45f_480p_{stamp}"
    ...
    print("Creating WanT2V pipeline once for Vbench10 SeaCache batch run")
    ...
    cmd = [
        args.python_bin,
        str(root_dir / "experiments" / "seacache_vbench10_50step_45f_480p" / "summarize_results.py"),
