...
def select_train_records(args) -> list[dict[str, object]]:
    ...
    if args.selected_source_ids:
        ...
        if args.prompt_limit <= 0:
            raise ValueError("--prompt_limit must be positive when sampling train prompts.")
        selected_ids = random.Random(args.random_seed).sample(train_ids, args.prompt_limit)
    outside_train = [source_id for source_id in selected_ids if source_id not in set(train_ids)]
    if outside_train:
        raise ValueError(f"Selected source IDs are not in train split: {outside_train}")
...
def main() -> None:
    ...
    parser.add_argument("--prompt_limit", type=int, default=10)
    ...
    if args.exp_root is None:
        ...
        args.exp_root = f"/hy-tmp/wan22_adaptive_seacache_train10_50step_45f_480p_{stamp}"
