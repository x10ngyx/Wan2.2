...
SESSION="${1:-adaptive_seacache_overhead_train5}"
WAIT_FOR_SESSION="${WAIT_FOR_SESSION:-adaptive_seacache_train15_test5}"
ROOT_DIR="/hy-tmp/work/Wan2.2"
PYTHON="/hy-tmp/miniconda3/envs/Wan2.2/bin/python"
STAMP="$(date +%Y%m%d_%H%M%S)"
EXP_ROOT="/hy-tmp/wan22_adaptive_seacache_overhead_train5_50step_45f_480p_${STAMP}"
...
tmux new-session -d -s "${SESSION}" "cd '${ROOT_DIR}' && \
  while tmux has-session -t '${WAIT_FOR_SESSION}' 2>/dev/null; do \
    echo \"waiting for tmux session ${WAIT_FOR_SESSION} to finish\"; \
    sleep 300; \
  done && \
...
echo "waiting for session: ${WAIT_FOR_SESSION}"
