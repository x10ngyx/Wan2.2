...
def trace_stats(trace_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    predictor_values = [
        ...
        if row.get("predictor_elapsed_seconds") not in (None, "", "None")
    ]
    return {
        ...
        "predictor_call_count": len(predictor_values),
        "predictor_elapsed_total_seconds": sum(predictor_values) if predictor_values else None,
        "predictor_elapsed_mean_seconds": (
            sum(predictor_values) / len(predictor_values) if predictor_values else None
        ),
        "predictor_elapsed_max_seconds": max(predictor_values) if predictor_values else None,
    }
...
def load_replay_threshold_trace(csv_path: Path) -> dict[tuple[str, str, int], float]:
    ...
    for row in rows:
        ...
        trace[key] = float(row["predicted_threshold"])
    return trace
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Adaptive SeaCache predictor overhead runner")
    ...
    if args.exp_root is None:
        ...
        args.exp_root = f"/hy-tmp/wan22_adaptive_seacache_overhead_train5_50step_45f_480p_{stamp}"
    ...
    for target_psnr in target_psnrs:
        ...
        for parent in ["online_adaptive", "replay_threshold"]:
            (exp_root / parent / label).mkdir(parents=True, exist_ok=True)
        for parent in ["psnr", "traces"]:
        ...
        rows.append(
            {
                ...
                "mean_psnr": psnr,
                "online_compute_elapsed_seconds": online_elapsed,
                "replay_compute_elapsed_seconds": replay_elapsed,
                "replay_overhead_seconds": (
                    online_elapsed - replay_elapsed
                    if online_elapsed is not None and replay_elapsed is not None
                    else None
                ),
    ...
    for record in records:
        ...
        for target_psnr in target_psnrs:
            ...
            online_method_id = f"online_adaptive_{label}"
            online_output = exp_root / "online_adaptive" / label / f"{source_id}.mp4"
            online_log = exp_root / "logs" / f"{online_method_id}_{source_id}.log"
            online_time = exp_root / "logs" / f"{online_method_id}_{source_id}.time"
            online_ffprobe = exp_root / "ffprobe" / f"{online_method_id}_{source_id}.json"
            online_psnr = exp_root / "psnr" / "online" / label / f"{source_id}.json"
            ...
            try:
                wan_text2video.SeaCacheTimestepCache = adaptive_factory
                if not (args.resume_existing and maybe_completed(
                    online_output,
                    online_time,
                    online_ffprobe,
                    online_psnr,
                    ...
                    print(f"Running online adaptive target {target_psnr} {source_id} seed {seed}")
                    ...
                    elapsed = parse_elapsed(online_log)
                    online_time.write_text(
                    ...
                    print(f"Skipping existing online adaptive target {target_psnr} {source_id}")

                online_trace_rows = json.loads(online_trace_json.read_text(encoding="utf-8"))
                online_stats = trace_stats(online_trace_rows)
                online_elapsed = read_time_file(online_time)
                online_mean_psnr = load_psnr(online_psnr)
                append_row(
                    record=record,
                    sample_id=sample_id,
                    source_id=source_id,
                    ...
                    elapsed=online_elapsed,
                    psnr=online_mean_psnr,
                    output=online_output,
                    log_path=online_log,
                    trace_json=online_trace_json,
                    trace_csv=online_trace_csv,
                    stats=online_stats,
                    online_elapsed=online_elapsed,
                )

                replay_factory = build_replay_seacache_factory(
                    load_replay_threshold_trace(online_trace_csv)
                )
                wan_text2video.SeaCacheTimestepCache = replay_factory
                replay_method_id = f"replay_threshold_{label}"
                replay_output = exp_root / "replay_threshold" / label / f"{source_id}.mp4"
                replay_log = exp_root / "logs" / f"{replay_method_id}_{source_id}.log"
                replay_time = exp_root / "logs" / f"{replay_method_id}_{source_id}.time"
                replay_ffprobe = exp_root / "ffprobe" / f"{replay_method_id}_{source_id}.json"
                replay_psnr = exp_root / "psnr" / "replay" / label / f"{source_id}.json"
                replay_psnr_log = exp_root / "psnr" / "replay" / label / f"{source_id}.log"
                replay_trace_json = exp_root / "traces" / "replay" / label / f"{source_id}.json"
                replay_trace_csv = exp_root / "traces" / "replay" / label / f"{source_id}.csv"
                ...
                if not (args.resume_existing and maybe_completed(
                    ...
                    print(f"Running replay threshold target {target_psnr} {source_id} seed {seed}")
                    replay_summary = generate_one(
                        args,
                        pipeline,
                        cfg,
                        prompt,
                        ...
                        replay_log,
                        make_seacache_config(args),
                        adaptive_factory=replay_factory,
                    )
                    replay_elapsed = parse_elapsed(replay_log)
                    replay_time.write_text(
                        f"elapsed_seconds={replay_elapsed if replay_elapsed is not None else ''}\n",
                        encoding="utf-8",
                    )
                    ...
                    print(f"Skipping existing replay threshold target {target_psnr} {source_id}")

                replay_trace_rows = json.loads(replay_trace_json.read_text(encoding="utf-8"))
                replay_stats = trace_stats(replay_trace_rows)
                replay_elapsed = read_time_file(replay_time)
                replay_mean_psnr = load_psnr(replay_psnr)
                decision_mismatch_count = sum(
                    1
                    for online_row, replay_row in zip(online_trace_rows, replay_trace_rows)
                    if online_row.get("decision") != replay_row.get("decision")
                )
                append_row(
                    record=record,
                    sample_id=sample_id,
                    source_id=source_id,
                    seed=seed,
                    method="replay_threshold",
                    target_value=target_value,
                    baseline_elapsed=baseline_elapsed,
                    elapsed=replay_elapsed,
                    psnr=replay_mean_psnr,
                    output=replay_output,
                    log_path=replay_log,
                    trace_json=replay_trace_json,
                    trace_csv=replay_trace_csv,
                    stats=replay_stats,
                    online_elapsed=online_elapsed,
    ...
    fieldnames = [
        ...
        "online_compute_elapsed_seconds",
        "replay_compute_elapsed_seconds",
        "replay_overhead_seconds",
