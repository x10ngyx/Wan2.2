...
def select_records(records, start: int, limit: int):
    ...
    if not selected:
        raise SystemExit("No VBench10 prompt records selected")
    return selected
...
def main():
    parser = argparse.ArgumentParser(description="Single-process VBench10 SeaCache UniPC Wan2.2 T2V runner")
    ...
    if not torch.cuda.is_available():
        raise SystemExit("CUDA is not available; VBench10 SeaCache UniPC runner requires GPU mode.")

    root_dir = Path(args.root_dir)
    if args.exp_root is None:
        stamp = time.strftime("%Y%m%d_%H%M%S")
        args.exp_root = f"/hy-tmp/wan22_seacache_unipc_vbench10_50step_45f_480p_{stamp}"
    ...
    records = load_prompt_records(Path(args.prompt_path))
    selected = select_records(records, args.prompt_start, args.prompt_limit)
    ...
    write_manifest_jsonl(exp_root / "manifests" / "selected_records.jsonl", selected)
    write_manifest_csv(exp_root / "manifests" / "selected_records.csv", selected)
    ...
    print("Creating WanT2V pipeline once for VBench10 SeaCache UniPC batch run")
    ...
    cmd = [
        args.python_bin,
        str(root_dir / "experiments" / "seacache_unipc_vbench10_50step_45f_480p" / "summarize_results.py"),
