...
def read_records(root: Path):
    path = root / "manifests" / "selected_openvid_records.jsonl"
    records = {}
...
def read_cache_summary(path: Path):
    ...
    matches = re.findall(r"ZEUS timestep cache summary: (\{.*\})", text)
    if not matches:
...
def main() -> None:
    ...
    for threshold_dir in threshold_dirs:
        ...
        for candidate_video in sorted(threshold_dir.glob("*.mp4")):
            sample_id = candidate_video.stem
            ...
            psnr = json.loads((root / "psnr" / threshold_label / f"{sample_id}.json").read_text(encoding="utf-8"))
            ...
            rows.append({
                ...
                "source_video_relative_path": record.get("video", ""),
                ...
                "zeus_threshold_elapsed_seconds": candidate_elapsed,
                ...
                "zeus_threshold_log": str(root / "logs" / f"{method_id}_{sample_id}.log"),
                "baseline_ffprobe": json.dumps(baseline_ffprobe, sort_keys=True),
                "zeus_threshold_ffprobe": json.dumps(candidate_ffprobe, sort_keys=True),
            })

    if not rows:
        raise SystemExit("No completed OpenVid ZEUS-threshold prompt pairs found")
    ...
    for threshold_label in sorted({row["threshold_label"] for row in rows}):
        ...
        total_candidate = sum(row["zeus_threshold_elapsed_seconds"] for row in subset)
        ...
        aggregate_rows.append({
            ...
            "total_zeus_threshold_elapsed_seconds": total_candidate,
    ...
    grouped_rows = []
    for threshold_label in sorted({row["threshold_label"] for row in rows}):
        for group_field in ["content_group", "portrait_group", "motion_group"]:
            group_values = sorted({row[group_field] for row in rows if row["threshold_label"] == threshold_label})
            for group_value in group_values:
                subset = [
                    row for row in rows
                    if row["threshold_label"] == threshold_label and row[group_field] == group_value
                ]
                total_baseline = sum(row["baseline_elapsed_seconds"] for row in subset)
                total_candidate = sum(row["zeus_threshold_elapsed_seconds"] for row in subset)
                mean_psnr_values = finite_values(row["mean_psnr"] for row in subset)
    ...
    grouped_csv = output.parent / "aggregate_by_threshold_and_group.csv"
    ...
    aggregate_json = {
        ...
        "grouped_aggregate_csv": str(grouped_csv),
