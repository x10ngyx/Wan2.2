...
DEFAULT_PROMPTS = REPO_ROOT / "test_sets" / "ali_10" / "prompts.jsonl"
...
def main():
    parser = argparse.ArgumentParser(description="Single-process Ali-10 SeaCache UniPC Wan2.2 T2V runner")
    ...
    if not torch.cuda.is_available():
        raise SystemExit("CUDA is not available; Ali-10 SeaCache UniPC runner requires GPU mode.")

    root_dir = Path(args.root_dir)
    if args.exp_root is None:
        stamp = time.strftime("%Y%m%d_%H%M%S")
        args.exp_root = f"/hy-tmp/wan22_seacache_unipc_ali10_50step_45f_480p_{stamp}"
    ...
    print("Creating WanT2V pipeline once for Ali-10 SeaCache UniPC batch run")
    ...
    cmd = [
        args.python_bin,
        str(root_dir / "experiments" / "seacache_unipc_ali10_50step_45f_480p" / "summarize_results.py"),
