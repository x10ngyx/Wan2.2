...
def main() -> None:
    ...
    rows = []
    for baseline_video in sorted((root / "baseline").glob("prompt_*.mp4")):
        prompt_id = baseline_video.stem.replace("prompt_", "")
        zeus_video = root / "zeus" / f"prompt_{prompt_id}.mp4"
        if not zeus_video.exists():
            continue

        baseline_elapsed = read_elapsed(root / "logs" / f"baseline_prompt_{prompt_id}.time")
        zeus_elapsed = read_elapsed(root / "logs" / f"zeus_prompt_{prompt_id}.time")
        psnr = json.loads((root / "psnr" / f"prompt_{prompt_id}.json").read_text(encoding="utf-8"))
        baseline_ffprobe = json.loads((root / "ffprobe" / f"baseline_prompt_{prompt_id}.json").read_text(encoding="utf-8"))
        zeus_ffprobe = json.loads((root / "ffprobe" / f"zeus_prompt_{prompt_id}.json").read_text(encoding="utf-8"))
        cache_summary = read_cache_summary(root / "logs" / f"zeus_prompt_{prompt_id}.log")
        ...
        rows.append({
            ...
            "zeus_reuse_count": sum_cache(cache_summary, "reuse"),
            "zeus_recompute_count": sum_cache(cache_summary, "recompute"),
            ...
            "zeus_log": str(root / "logs" / f"zeus_prompt_{prompt_id}.log"),
