...
SESSION="${1:-adaptive_seacache_train15_test5}"
ROOT_DIR="/hy-tmp/work/Wan2.2"
PYTHON="/hy-tmp/miniconda3/envs/Wan2.2/bin/python"
STAMP="$(date +%Y%m%d_%H%M%S)"
EXP_ROOT="/hy-tmp/wan22_adaptive_seacache_train15_test5_50step_45f_480p_${STAMP}"
...
tmux new-session -d -s "${SESSION}" "cd '${ROOT_DIR}' && \
  ...
  '${PYTHON}' experiments/adaptive_seacache_train15_test5_50step_45f_480p/run_batch.py \
    --root_dir '${ROOT_DIR}' \
    --python_bin '${PYTHON}' \
    --exp_root '${EXP_ROOT}' \
