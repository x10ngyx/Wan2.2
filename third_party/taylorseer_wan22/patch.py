...
def install_taylorseer(model, cache, stage_key):
    ...
    base_model._taylorseer_cache = cache
    base_model._taylorseer_stage_key = stage_key
    base_model._taylorseer_stream = None
    base_model._taylorseer_step_type = None
    attach_parent(base_model)
    for layer_index, block in enumerate(base_model.blocks):
...
def set_taylorseer_context(model, stream, step_type):
    base_model = unwrap_model(model)
    base_model._taylorseer_stream = stream
    base_model._taylorseer_step_type = step_type
